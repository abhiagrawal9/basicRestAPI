...
app.get(`${BASE_URL}/users/:id`, (req, res) => {
  ...
  let user = userInfoData.find(
    (user) => user.id === Number.parseInt(requestId)
  );
  if (user) {
    res.json(user);
  } else {
...
app.post(`${BASE_URL}/users`, (req, res) => {
  let isExist = userInfoData.find(
    (user) => user.id === Number.parseInt(req.body.id)
  );
  const user = {
...
app.delete(`${BASE_URL}/users/:id`, (req, res) => {
  ...
  } else res.send('User does not exist with given id.');
});
